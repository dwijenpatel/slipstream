...
| Expert-cache policy | `lfu`, `lru` | `--expert-cache-policy` | `lfu` | Replacement policy for the slots. Monotonic LFU keeps use counts for the life of the process, so on a long session experts that were hot early stay resident after they stop being used. Replayed offline on a recorded ten-turn coding session (2026-09-05, 18,772 decode tokens): at 64 slots LFU hit 69.1 percent, LRU 75.9, the offline optimum 87.8; on a single 3k prompt the two tie. See `playbook/route_replay.py`. |
